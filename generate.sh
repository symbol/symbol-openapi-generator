...
buildPython() {
  ...
  # Test
  echo "pytest -v --color=yes --exitfirst --showlocals --durations=5"
  PYTHONPATH=".:${PYTHONPATH}" pytest -v --color=yes --exitfirst --showlocals --durations=5

  # Publish/Release
  REPO="pypi"   # default repo
  UPLOAD=true   # convenient var when you need to disable uploading of the artifact

  if [[ $OPERATION == "test" ]] || [[ $OPERATION2 == "test" ]] || { [[ -n ${TEST_PYPI_USER} ]] && [[ -n ${TEST_PYPI_PASS} ]]; }; then
    REPO="testpypi"
  ...
  if [[ $UPLOAD == true ]]; then
    ...
    echo "UPLOAD=$UPLOAD ARTIFACT_ID='$ARTIFACT_ID' PACKAGE_VERSION='$PACKAGE_VERSION' REPO='$REPO'"
  fi
